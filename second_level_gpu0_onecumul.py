...
PATH = './second_level/'+'rake_onecumul.tar'
writer = SummaryWriter("./runs/onecumul/")
...
def do_eval(steps):
    ...
    for data in tqdm(valid_dataset[:1000]):
...
def trainer(LAST_STEP):
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataset[LAST_STEP%len(train_dataset):],0):
