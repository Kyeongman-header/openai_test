   ...
   def generate(self, memory,input_ids,attention_mask,decoder_input_ids,decoder_attention_mask,labels,prev_predictions,conti_prev_predictions,conti_keyword_prev_predictions,order,whole,intro,tail,use_memory,use_cumulative,use_rake):#prompt_ids,prompt_attention):
        ...
        for b in range(batch_size):
            ...
            outputs.append(self.bart.generate(max_length=350,memory=one_memory,
                        ...
                        #encoder_no_repeat_ngram_size=3,
                        repetition_penalty=1.4,length_penalty=-1.0,early_stopping=True,context=one_context,alpha=one_alpha,beta=one_beta))
...
# with open("pickle_data/"+"bart_train_"+dataset_dir+"/level_2_whole.pickle","rb") as fi:
...
def trainer(LAST_STEP,train_dataset,NumPar,lr_scheduler,progress_bar,epoch):
    ...
    #print(prev_predictions)
        ...
        # input_ids 맨 앞에 이전 preceding context를 합친다.
            ...
            # 주의!!! bart는 이렇게 하지만 bart는 위에 코드로 해야함!
            ...
            # bart에서는 output이 input을 싸그리 포함하고 있으므로,
    ...
    # print statistics
        ...
        if i % 3000 == 0 and i!=0:    # print every 2000 mini-batches
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        for i in range(LAST_PARAG,30): # 최대 30개 문단까지 있다.
            ...
            if dataset_dir !="whole":
                ...
                with open("pickle_data/"+"bart_valid_"+"wp_rake"+"/level_2_" + str(i) + ".pickle","rb") as fi:
                    valid_dataset = pickle.load(fi)
                with open("pickle_data/"+"bart_valid_"+"reedsy_rake"+"/level_2_" + str(i) + ".pickle","rb") as fi:
                    valid_dataset += pickle.load(fi)
                with open("pickle_data/"+"bart_valid_"+"booksum_rake"+"/level_2_" + str(i) + ".pickle","rb") as fi:
                    valid_dataset += pickle.load(fi)
