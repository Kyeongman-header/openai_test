...
CUMUL_NUM=3
train_LIST=['train_bart','train_bart-2','train_reedsy','booksum/train']
valid_LIST=['valid_bart','valid_reedsy','booksum/dev']
...
def do_eval(steps,valid_dataset):
...
def trainer(LAST_STEP):
    ...
    whole_train_length=0
    

    for name in valid_LIST:
        with open("pickle_data/"+name+"/level_2.pickle","rb") as fi:
                valid_dataset+= pickle.load(fi)
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        for name in train_LIST:
            ...
            for i, data in enumerate(tqdm(train_dataset[LAST_STEP:])):
            ...
            # print statistics
                ...
                if i % 3000 == 2999:    # print every 2000 mini-batches
                    ...
                    writer.add_scalar("Loss/train", running_loss/whole_count_for_save, whole_train_length + i)
                ...
                if i % 200000 == 199999:
                    do_eval(whole_train_length+i,valid_dataset)
                    writer.flush()

            conti_first=True
            whole_train_length+=train_length
