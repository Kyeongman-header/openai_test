...
soplot_token_tensor=torch.LongTensor([[soplot_id]]).to('cuda:1')
eoplot_token_tensor=torch.LongTensor([[eoplot_id]]).to('cuda:1')
soprev_token_tensor=torch.LongTensor([[soprev_id]]).to('cuda:1')
eoprev_token_tensor=torch.LongTensor([[eoprev_id]]).to('cuda:1')
sep_token_tensor=torch.LongTensor([[sep_id]]).to('cuda:1')
# by_token_tensor=torch.LongTensor([[by_id]]).to('cuda:1')
intro_token_tensor=torch.LongTensor([[intro_id]]).to('cuda:1')
body_token_tensor=torch.LongTensor([[body_id]]).to('cuda:1')
tail_token_tensor=torch.LongTensor([[tail_id]]).to('cuda:1')
front_token_tensor=torch.LongTensor([[front_id]]).to('cuda:1')
middle_token_tensor=torch.LongTensor([[middle_id]]).to('cuda:1')
ending_token_tensor=torch.LongTensor([[ending_id]]).to('cuda:1')
...
class Network(nn.Module): 
   ...
   def forward(self, memory,input_ids,attention_mask,decoder_input_ids,decoder_attention_mask,labels,prev_predictions,conti_prev_predictions,order,whole,intro,tail):#prompt_ids,prompt_attention):
       ...
       #print(input_ids.shape)
    #    order_token_tensor=torch.LongTensor([[order]]).to('cuda:1')
    #    whole_token_tensor=torch.LongTensor([[whole]]).to('cuda:1')
        if intro:
        ...
        if USE_CUMULATIVE is False or input_ids.shape[1]+conti_prev_predictions.shape[1]+5 > 1020:
    ...
    for data in tqdm(random.sample(valid_dataset,1000)):
        ...
        print(loss)
            ...
            if USE_MEMORY is False:
                memory = torch.zeros_like(torch.empty(1,1024,config.d_model)).to('cuda:1')
            ...
            with torch.no_grad():
                ...
                ddd=dd[:,:-1].to('cuda:1')
                dd_attention_mask=dd_attention_mask[:,:-1].to('cuda:1')
                # input_ids 맨 앞에 이전 preceding context를 합친다.
                dlabel=dd[:,1:].to('cuda:1')
            ...
            if USE_CUMULATIVE:
                ...
                cumul_prev_predictions.insert(0,tokenizer(keywordsSTR,return_tensors='pt'))
