...
cumul_num=int(sys.argv[3]) # 3

no_ibt=int(sys.argv[4])
no_fme=int(sys.argv[5])
eval_self_bleu=int(sys.argv[6])
dataset_dir=sys.argv[7]

gpu_name=sys.argv[8] # cuda:0
debug = int(sys.argv[9]) # 1
if debug ==1:
    debug=True
else:
    debug=False
...
class Network(nn.Module): 
   ...
   def forward(self, memory,input_ids,attention_mask,decoder_input_ids,decoder_attention_mask,labels,prev_predictions,conti_prev_predictions,order,whole,intro,tail,use_cumulative,use_memory):#prompt_ids,prompt_attention):
    ...
    #    whole_token_tensor=torch.LongTensor([[whole]]).to(gpu_name)
        ...
        outputs = self.bart(input_ids = None,inputs_embeds=inputs_embeds,attention_mask = attention_mask,decoder_input_ids = decoder_input_ids,decoder_attention_mask=decoder_attention_mask,labels=labels,output_hidden_states=True,memory=memory,cumulation=cumulation,alpha=0.5,beta=0.5)
   ...
   def generate(self, memory,input_ids,attention_mask,decoder_input_ids,decoder_attention_mask,labels,prev_predictions,conti_prev_predictions,order,whole,intro,tail,use_memory,use_cumulative):#prompt_ids,prompt_attention):
       ...
       #input_ids=torch.cat((prompt_ids,input_ids),1)
        ...
        return self.bart.generate(max_length=250,memory=memory,inputs_embeds=inputs_embeds,attention_mask=attention_mask,
                ...
                repetition_penalty=3.5,early_stopping=True,cumulation=cumulation,alpha=0.5,beta=0.5),memory
