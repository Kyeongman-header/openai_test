...
class Network(nn.Module): 
   def __init__(self, shared, vocab_size, d_model,bart,bert,bert_config): 
   ...
   def forward(self, memory,input_ids,attention_mask,decoder_input_ids,decoder_attention_mask,labels,prev_predictions,conti_prev_predictions,conti_keyword_prev_predictions,order,whole,intro,tail,use_cumulative,use_memory,use_rake):#prompt_ids,prompt_attention):
       #memory states update.
        ...
        #     # bart STYLE!! BART에서는 EOS 토큰을 뺄 필요도 없고, PAD만 날리면 되며, 애초에 DECODER INPUT IDS랑 합치지도 않는다.
        ...
        #     residual=input_id
        #     # bart 토크나이저는 eos 토큰을 따로 추가하지 않는다고 한다. eos 토큰은 굳이 필요 없는 듯 하다.
   ...
   def generate(self, memory,input_ids,attention_mask,decoder_input_ids,decoder_attention_mask,labels,prev_predictions,conti_prev_predictions,conti_keyword_prev_predictions,order,whole,intro,tail,use_memory,use_cumulative,use_rake):#prompt_ids,prompt_attention):
        ...
        for b in range(batch_size):
            ...
            # 주의! bart는 generation에서 embedding을 input으로 못 넣음
            ...
            # outputs.append(self.bart.generate(max_length=250,memory=memory[b],inputs_embeds=inputs_embeds[b],attention_mask=attention_mask[b],
            #         #num_beams=4,
            #         do_sample=True,
...
shared = bart.get_input_embeddings()
...
model = Network(shared,vocab_size, d_model,bart, bert,bert_config).to(gpu_name)
...
eval_dir='bartGenerations/'+save_dir+'/lengthcontrol'
...
def do_eval(steps,dataset,NumPar,eval_num,eval_first):
    ...
    #print(prev_predictions)
        ...
        # input_ids 맨 앞에 이전 preceding context를 합친다.
            ...
            # bart의 경우 generation output이 input ids를 포함하고 있다.
            ...
            for length,output in enumerate(outputs):
                ...
                # print(output.shape)
                # output=output[:,input_lengths[length]:] # gpt에서만 이렇게 처리.
            ...
            for b in range(batch_size):
                wr.writerow([str(steps),str(index),batch_input_text[b],predictions[b],predictions[b]]) # 전부 plain string이다.
            
eval_first=True
if dataset_dir !="whole":
    # 3 param의 keyword를 임의로 사용한다.
    if dataset_dir=='reedsy_rake': # reedsy rake는 test dataset이 없다
        with open("pickle_data/"+"bart_valid_"+dataset_dir+"/level_2_" + "3" + ".pickle","rb") as fi:
            test_dataset = pickle.load(fi)
    else:
        with open("pickle_data/"+"bart_test_"+dataset_dir+"/level_2_" + "3" + ".pickle","rb") as fi:
            test_dataset = pickle.load(fi)
else: # whole dataset train.
    with open("pickle_data/"+"bart_test_"+"wp_rake"+"/level_2_" + "3" + ".pickle","rb") as fi:
        test_dataset = pickle.load(fi)
    with open("pickle_data/"+"bart_valid_"+"reedsy_rake"+"/level_2_" + "3" + ".pickle","rb") as fi:# reedsy rake는 test dataset이 없다
        test_dataset += pickle.load(fi)
    with open("pickle_data/"+"bart_test_"+"booksum_rake"+"/level_2_" + "3" + ".pickle","rb") as fi:
        test_dataset += pickle.load(fi)
...
for i in range(LAST_PARAG,30): # 최대 100개 문단까지 있다.    
    ...
    do_eval(steps=i,dataset=test_dataset,NumPar=i,eval_num=40,eval_first=eval_first)
    eval_first=False
